...
以下の手順に従って、ユーザーにインタビューを行い、ニーズを深掘りしてください。

これまでの会話コンテキスト: {context}

## インタビューアルゴリズム
1. 「これから{theme}を買う場合、どんな特徴を持った{theme}を求めますか？（1つ挙げてください）」と質問する
2. ユーザの回答から特徴を抽出する
...
5. 製品名がない場合
    ...
    - 「購入したいと思うか？（yes/no）」と質問する
      - yesの場合 -> 1.に戻る
      - noの場合 -> 「なぜ購入しないのか、理由をすべて教えてください」と質問する
        - ラダリング：理由を聞き、それぞれ「なぜ◯◯と思うのか？」と深掘りする（「わからない」または深掘りする必要がない場合1.に戻る）
        - 1.に戻る
...
質問者: （ユーザーは既存のスマートウォッチを認識していない。 -> 次に、購入意向を確認）それらのスマートウォッチを購入する予定はありますか？
